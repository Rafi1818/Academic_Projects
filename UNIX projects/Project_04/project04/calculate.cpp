...
int main(int argc, char* argv[]) {
    try {
        ...
        if      (command == "sum")
            ...
            stringstream s;
            s   << endl << endl <<
            "Invalid Command: command" << endl << "Usage:" << endl << "calculate < sum | average | greatest | least > filename.txt " << endl << 
            "filename - a file that contrains integers, 1 integer per line " << "Example:" << endl << "calculate sum filerandom.txt" << endl << endl;
        throw domain_error(LineInfo(s.str(), __FILE__, __LINE__));
