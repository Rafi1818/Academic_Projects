class HashTableBuilder {
    ...
    private Element createNewElement(String keyword, Article recordToAdd) { // Creates a new Element to be added to the HashTable
        return new Element(keyword, recordToAdd);
    }

    void insert(String keyword, FileData fd) {
        Article recordToAdd = new Article(fd.id, fd.title, fd.author, null); // Creates a new Record object with the relevant information
        int index = find(keyword); // Finds the correct index for the Record using the find method
        if (index == -1)
            insert(keyword, recordToAdd); // Calls the insert method if there is a redundancy in the keyword
        else {
            recordToAdd.next = hashTable[index].head;
            hashTable[index].head = recordToAdd; // Adds Record to the HashMap if the keyword is unique
        }
    }

    private void insert(String keyword, Article recordToAdd) { // Used to handle redundancies
        ...
        hashTable[index] = createNewElement(keyword, recordToAdd); // Adds the element to the specified index
    }
